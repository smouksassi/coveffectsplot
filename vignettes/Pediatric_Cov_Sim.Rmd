...
# ggsave("ped1.png", device="png",type="cairo-png",width= 6, height = 5,dpi=72)
```
...
#                "(0.75, 0.95]","(0.95, 1]")
#   )+
#   labs(x="Reference Subject\nStandardized PK Parameters with BSV",y="")+
#   scale_x_log10()+
...
The NHANES website provides a csv file containning the smoothed growth charts distribution parameters at specific ages for boys and girls. The `gamlss.dist::rBCCG` function is used to show how we can use these parameters to generate a realistic pediatric Age/Weight/Sex distribution. 
...
#   guides(colour=guide_legend(override.aes = list(alpha=1,size=0.5)))
# 
#  ggsave("pedmcov3.png", device="png",type="cairo-png",
...
# ggsave("pedcov1.png", device="png",type="cairo-png",width= 8, height = 5,dpi=72)
 # wtvsageplot +theme_bw(base_size=18) + facet_grid(SEXLABEL~.,switch="y")
 # ggsave("pedcov1b.png", device="png",type="cairo-png",width= 5, height = 6,dpi=72)
