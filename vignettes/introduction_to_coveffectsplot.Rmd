...
The `coveffectsplot` R  package aims to facilitate the creation of forest plots using the `forest_plot` function.
...
1. Fixed effects (POPCL, dWTdCL and dSexdCL from the model) : Always
2. Random effects ? (Omega matrix from the model): To illustrate range of possible outcomes in the population coupled with realistic covariate distribution
3. Uncertainty on fixed and random effects ? (from model varcov or bootstrap): Always
4. Covariate values or distributions ? (modeled data or a database): Univariate to determine relative importance, multivariate for range of possible outcomes
5. Residual error and its uncertainty ? (from model): only when there is interest in the predicted concentrations distributions.
...
Finally, since the BSV cannot be controlled for, showing the distribution of the BSV is important to contrast and compare with the estimated covariate related effects as this will allow us to understand where a random subject given a known set of covariates (e.g. at reference values) could possibly belong.

In many situations, the assumption of varying one covariate at a time does not hold and  we recommend to also conduct simulations with realistic distribution of correlated covariates.  

Both methods are complementary and the first shows us the relative importance of covarites while the second will illustrate the full distribution of possible outcomes in the population,
...
ggplot(dflong,aes(x=value,fill=key))+
...
ggplot(dfeffects)+
  geom_density(aes(x=REF,y=..scaled..,col="a.Uncertainty\nRSE=15%"))+
