...
In this section we compute the PK parameters of interest, provide a plot of the parameters as well as of the standardized ones. We also summarize and report the BSV as ranges of 50 and 90% of patients for each PK parameter. Later on we might chooose to include these ranges in the coveffectsplot or not.
...
covdatasim <- read.csv("./covdatasim.csv")
...
plotranges<- ggplot(outcovcomb, aes(time,CP,col=SEX,fill=SEX ) )+
  ...
  labs(linetype="Sex",colour="Sex",fill="Sex",caption ="Simulation with Full Covariate Distribution\nwith BSV without Uncertainty\n95% (Covariate Effects + BSV) Percentiles",
       x = "Time (h)", y="Plasma Concentrations")+
  coord_cartesian(ylim=c(0,3.5))
...
for(i in 1:nsim) {
  ...
  out <- modcovsim %>%
    data_set(data.all) %>%
    #zero_re() %>%
    #omat(RxODE::cvPost(2000, matrix(c(0.09,0.01,0.01,0.09), 2, 2), type = "invWishart")) %>%  # unc on bsv unocmment and increase nsim for paper
...
png("./coveffectsplot_full.png",width =9.5 ,height = 6,units = "in",res=72)
