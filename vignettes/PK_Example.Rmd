...
In this section we compute PK parameters using noncompartmental analysis (NCA), provide a plot of the parameters as well as of the standardized ones. We also summarize and report the BSV as ranges of 50 and 90% of patients for each PK parameter.
...
Similar to an earlier section, we compute the NCA parameters by patient and by replicate standardize by the median and provide a plot. We add some data manipulation to construct more informative labels that will help in the plotting.
...
Here we provide an alternative visual summary of the PK parameters. It shows the distribution, quantiles of interest and a simpler median, 90% pointinterval summary right below it. It isolates each covariate effects in one panel keeping the reference on its own. It is exactly the same data as the boxplots. Which visual presentation do you prefer? Which one enables you to clearly see and compare the covariate effects?  
