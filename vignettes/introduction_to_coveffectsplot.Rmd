...
Finally, showing the distribution of the unexplained between subject variability (BSV) is important to contrast with the magnitude of the effects explained by the covariate.

## Simulating data and modeling outputs

We will assume that the model fit had estimated parameters with relative standard errors of 10%. In practice, we can also get these relative standard errors from a bootstrap resampling procedure rather than relying on the asymptotic variance covariance matrix. For this vignette, a simulation from a multivariate normal distribution was used with n= 1000 was used. The five first rows are shown in the table below. 
For simplicity, we will also assume that there were equal number of SEX = 1 (Man) and SEX = 0 (Woman) and that men and women had mean weights of 75 and 65 kg and the same standard deviation of 15 kg. 
...
ggplot2::ggplot(data= CLBSVdistribution)+
 ggplot2:: geom_density(ggplot2::aes(x=CLBSV,
  ...
  ggplot2::labs(colour="",fill="BSV Coverage",x="BSVCL/Reference CL",
                title="Illustrating 30% BSV Effects")+
  ggplot2::guides(fill = ggplot2::guide_legend(override.aes =  list(alpha = 0.1)  ))




knitr::kable(t(dfbsv),row.names=FALSE)
```
...
dfeffects<- dfeffects[,c("SEX_FEMALE_WT_50","SEX_FEMALE_WT_90","SEX_Male_WT_70",
                         "SEX_Male_WT_90","REF")]
...
dfeffectslongsummaries$label <- c("95% of patients","70 kg/Woman","70 kg/Man",
                                  "50 kg/Woman", "90 kg/Woman")
