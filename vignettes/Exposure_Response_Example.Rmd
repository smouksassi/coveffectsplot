...
Here instead of using a limited expand grid we construct a full grid of Dose levels, Weights and Severity,
...
a <- ggplot(data=data.frame(iter_sims),aes(AUC, P1,
 ...
 b <-  ggplot(iter_sims,
       ...
       facet_grid(.~ WT,labeller = labeller(WT=wt.labs, SEV = SEV_names))+
    geom_vline(xintercept = vlines)+
   stat_slab(scale = 4, alpha=0.5)+
   stat_pointinterval(.width = c(.5,.90))+
     theme_bw(base_size = 14)+
    theme(legend.position="none",
...
```{r plot6, collapse=TRUE, include=FALSE}
