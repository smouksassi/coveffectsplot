...
```
### Simulate the reference subject with between subject variability
...
### Compute Typical Subject NCA PK parameters

```{r computenca , fig.width=6 }
...
Based on your observed covariate data we compute percentiles that we will use in this section.
...
### Simulate without uncertainty and without BSV
...
We will invent a varcov matrix by assuming 15% relative standard errors and 0.2 correlations across the board.
...
```
We then simulate a 1000 set of parameters using a multivariate normal. Unless the model was written in a way to allow unconstrained parameter values, care should be taken to make sure the simulated parameters make sense. Alternatively you can use the set of parameters from a bootstrap run.
...
### Compute and Plot NCA PK parameters
...
### Compute summaries and present the results
...
### Adding the BSV and Fine tuning
...
png("./coveffectsplot4.png",width =8.5 ,height = 6,units = "in",res=72)
egg::ggarrange(
  main_plot,
  table_plot,
  nrow = 1,
...
png("./coveffectsplot5.png",width =9 ,height = 6,units = "in",res=72)
