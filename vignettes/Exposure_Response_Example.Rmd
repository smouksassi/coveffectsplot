...
probplot<- ggplot(simout, aes(AUC,DV,linetype=factor(SEV))) +
  ...
  geom_label(data=data.frame(
    x=8.7,y=TypicalProb,label=paste(round(100*TypicalProb,1),"%"),SEV=0),
    aes(x=x,y=y,label=label),fill="transparent")+
  geom_label(data=data.frame(
    x=0.32,y=0.1,label=round(100*0.1,1),SEV=0),
    aes(x=x,y=y,label=label),fill="transparent")+
...
#     axis.title.y    = element_blank())
# (probplot + pbsvranges)
#  ggsave("Figure_8_1.png", device="png",type="cairo-png",width = 10, height = 5)
```
